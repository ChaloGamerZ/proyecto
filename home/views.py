...
def agregar_prestamo(request):
	...
	Detalle_PrestamoFormSet = inlineformset_factory(Prestamo,Detalle_Prestamo,min_num=1, form=agregar_DPrestamoF,max_num=50)
	if request.method=='POST':
...
def ver_prestamo(request,id_prest):
	p =Prestamo.objects.get(id=id_prest)
	Detalle_PrestamoFormSet = inlineformset_factory(Prestamo,Detalle_Prestamo, form=agregar_DPrestamoF)
	if request.method=='POST':
...
def devolucion_prestamo(request):
	Detalle_PrestamoFormSet = inlineformset_factory(Prestamo,Detalle_Prestamo, form=agregar_DPrestamoF)
	if request.method=='POST':
		formulario=agregar_prestamoF(request.POST)
		formset=Detalle_PrestamoFormSet(request.POST)
		...
		formulario=agregar_prestamoF()
		formset=Detalle_PrestamoFormSet()
	return render(request,'devolucion_prestamo.html',locals(),{'formset': formset})
...
def eliminar_DetallePrestamo(request,id_Dprest):
	Dp=Detalle_Prestamo.objects.get(id=id_Dprest)
	Dp.delete()
	return redirect('/ListaD/')
